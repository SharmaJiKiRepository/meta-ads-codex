...
for use by `/meta-ads create`, `/meta-ads generate`, and `/meta-ads photoshoot`.

Brand DNA extraction is OBSERVE and LISTEN made concrete (see the
**10-Principle Thinking Framework** in `meta-ads/references/thinking-framework.md`).
...
|---------|-------------|
| `/meta-ads dna <url>` | Full brand extraction → `brand-profile.json` |
| `/meta-ads dna https://acme.com --quick` | Fast extraction (homepage only) |
...
These serve as visual style references during `/meta-ads generate`; the same approach
...
```bash
python ~/.codex/skills/meta-ads/scripts/capture_screenshot.py [url]
```
Saves: `./brand-screenshots/{domain}_homepage.png`

2. **Product or services page**:
```bash
python ~/.codex/skills/meta-ads/scripts/capture_screenshot.py [url]/products
```
Saves: `./brand-screenshots/{domain}_product.png`

3. **About page** (brand personality):
```bash
python ~/.codex/skills/meta-ads/scripts/capture_screenshot.py [url]/about
```
...
Read `~/.codex/skills/meta-ads/references/brand-dna-template.md` for the exact schema.
...
Run `/meta-ads create` to generate campaign concepts from this profile.
```
