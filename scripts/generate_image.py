...
ASPECT_RATIOS = {
    ...
    "4:3":   (1200, 900),
    "3:4":   (900, 1200),
    "1.91:1": (1200, 628),  # Meta right column / link preview
    "4:1":   (1200, 300),   # wide banner (rarely used on Meta)
...
def run_batch(batch_file: str, output_dir: str, provider: str, model: str | None, api_key: str, as_json: bool) -> None:
    ...
    Batch file format:
    [
        {"prompt": "...", "ratio": "9:16", "output": "reels-ad.png"},
    ...
    for i, job in enumerate(jobs):
        ...
        if reference_image and Path(reference_image).suffix.lower() not in _ALLOWED_IMAGE_EXTENSIONS:
            print(f"  [WARN] Skipping invalid reference image: {reference_image}", file=sys.stderr)
        ...
        try:
            ...
            print(f"  [OK] Saved to {output_path} ({width}x{height})", file=sys.stderr)
        except Exception as e:
...
Examples:
  ...
  python generate_image.py "Reels ad" --ratio 9:16 --provider gemini
  python generate_image.py --batch prompts.json --output-dir ./ad-assets/
...
""",
    ...
    if args.json:
        ...
        print(f"[OK] Generated {width}x{height} image -> {output_path}")
